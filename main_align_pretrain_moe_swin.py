...
def main(args):
    ...
    transform_train = DataAugmentationV1((args.height,args.width),
    ...
    pretrained_student = torch.load("pretrained_models/solider/swin_small.pth", map_location='cpu')['student']
    new_ckpt = {}
    for k, v in pretrained_student.items():
        ...
        new_ckpt[new_key] = v
    missing_keys, unexpected_keys = student.backbone.init_from_pretrain(new_ckpt, strict=False)
    print(f"Student Missing keys: {missing_keys}")
    print(f"Student Unexpected keys: {unexpected_keys}")
...
def train_one_epoch(student, teacher, ema_teacher, ema_teacher_without_ddp, csm_kd_loss, momentum_schedule,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            # pred_t = teacher(samples[0], meta) # solider
            pred_t = teacher.forward_w_crowd(samples[0], meta) # path
            ...
            if epoch < 20:
                ...
                m_loss['moe_loss'] = 0.01 * moe_loss # 0.1
